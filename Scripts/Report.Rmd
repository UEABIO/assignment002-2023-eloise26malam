...
Finding an appropriate linear model for this data set required testing out multiple iterations. The response variable in this model is song duration, looking at diet , weight change, and size as possible explanatory variables. Initial exploratory plots illustrated that both size and diet may have an effect on weight change, so these interactions were taken into account in the model. However, both interactions as well as the size variable were dropped from the model as they did not improve its fit.

Performance checks of this initial model showed some room for improvement, however performing a log transformation on the data produced an even worse fit. For this reason, the final model used has song duration as a response variable based on diet and weight change.
